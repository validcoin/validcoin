...
#if defined(NDEBUG)
# error "Validcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x2429d0f7d84946273bc793cfee6a0482540755a490827dd8d511884b5ef8bd0a");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Validcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Validcoin Signed Message:\n";
...
{
    // Validcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 600000); // Validcoin: 600k blocks in ~3.5 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1.0 * 24 * 60 * 60; // Validcoin: 1 day
static const int64 nTargetSpacing = 0.5 * 60; // Validcoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Validcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Validcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x2aec138a1b8101b8a9319f4fb8686a2f5e3deece037707acff6371613afdf04c");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Baruch atah Adonai, Eloheinu melech haolam, hamotzi lechem min ha-aretz";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 60 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("045bb4254a49b4cfddc3afd4f4cff1b24b579ad38eb5416cb1624aa631d97a834b02ef6d3ade5e3484e9c608c320deabba24d5fabc4a17d698a10f35cb647f5674") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1522437682;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085298345;

        if (fTestNet)
        {
            block.nTime    = 1522437630;
            block.nNonce   = 385689391;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x551a399cdb1b9649ae411582e017e7bf0d46706c0439079b517e7b0d51f46d8d"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfc, 0xca, 0xb3, 0xd7 }; // Validcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("ValidcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("ValidcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static ValidcoinMiner(CWallet *pwallet)
{
    printf("ValidcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("validcoin-miner");
    ...
    try { loop {
        ...
        printf("Running ValidcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("ValidcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&ValidcoinMiner, pwallet));
}
